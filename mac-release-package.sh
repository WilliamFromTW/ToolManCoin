...
PACKAGE=KFO-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=KFO-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} KafeiouCoin.exe KafeiouCoinservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe KafeiouCoin.exe
cp installer/lib/JavaExe.exe KafeiouCoinservice.exe
cp -a ${FILES} nxt
...
/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents/Home/bin/javapackager -deploy -outdir . -outfile KFO-client -name KFO-installer -width 34 -height 43 -native dmg -srcfiles ${PACKAGE}.jar -appclass com.izforge.izpack.installer.bootstrap.Installer -v -Bmac.category=Business -Bmac.CFBundleIdentifier=org.nxt.client.installer -Bmac.CFBundleName=KFO-Installer -Bmac.CFBundleVersion=${MACVERSION} -BappVersion=${MACVERSION} -Bicon=installer/AppIcon.icns -Bmac.signing-key-developer-id-app="Developer ID Application: Stichting NXT (YU63QW5EFW)" > installer/javapackager.log 2>&1
